  ...
  return (
    <div className="min-h-screen bg-[#f8fafb] pt-28 pb-20">
      ...
      <main className="max-w-7xl mx-auto px-6 lg:px-10 mt-12">
        ...
        {heroPost && (
          <div className="mb-12">
            <article className="group bg-white rounded-3xl overflow-hidden border border-gray-200/80 shadow-md hover:shadow-2xl transition-all duration-300 grid grid-cols-1 lg:grid-cols-12 gap-0">
              <Link href={`/blog/${heroPost.slug}`} className="relative lg:col-span-7 aspect-[16/10] lg:aspect-auto bg-gray-100 block overflow-hidden">
                ...
                {heroPost.category && (
                  <span className="absolute top-6 left-6 bg-[#006672] text-white text-xs font-extrabold px-3.5 py-1.5 rounded-full shadow-md">
              ...
              <div className="lg:col-span-5 p-8 lg:p-10 flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-4 text-xs font-medium text-gray-400 mb-4">
                    <span className="flex items-center gap-1 text-[#006672] font-semibold">
                  ...
                  <h2 className="text-2xl md:text-3xl font-extrabold text-gray-900 group-hover:text-[#006672] transition-colors leading-tight mb-4">
                    <Link href={`/blog/${heroPost.slug}`}>{heroPost.title}</Link>
                  </h2>

                  {heroPost.summary && (
                    <p className="text-base text-gray-600 line-clamp-4 leading-relaxed mb-6">
                      {heroPost.summary}
                    </p>
                  )}
                </div>
                ...
                <div className="pt-4 border-t border-gray-100 flex items-center justify-between mt-auto">
                  <div className="flex flex-wrap gap-1.5">
                    {heroPost.tags.slice(0, 3).map((t: string, idx: number) => (
                      <span key={idx} className="bg-emerald-50 text-[#006672] text-xs font-bold px-2.5 py-1 rounded-lg">
                        #{t}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="p-6 flex-1 flex flex-col justify-between">
                    ...
                    <div className="pt-4 border-t border-gray-100 flex items-center justify-between mt-auto">
                      <div className="flex items-center gap-1.5 text-xs text-gray-500">
                        {post.tags.slice(0, 2).map((t: string, idx: number) => (
                          <Link
                            key={idx}
                            href={`/blog?tag=${encodeURIComponent(t)}`}
                            className="bg-gray-100 hover:bg-emerald-50 hover:text-[#006672] px-2 py-0.5 rounded text-gray-600 font-medium transition-colors"
                          >
                            #{t}
                          </Link>
                        ))}
